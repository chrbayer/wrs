...
| S-04b | Starting fighters are distributed from a fixed pool (`FIGHTERS_PER_PLAYER` × player count), inversely proportional to neighbor production score (sum of neighbor production rates, including second-hop neighbors at half weight; weaker neighbors → more fighters, clamped to `MIN_START_FIGHTERS`–`MAX_START_FIGHTERS`) | ✅ Done |
...
| SL-21a | Active shield lines are drawn as colored lines (owner's player color) between the two endpoint systems, shortened to star radius. Line width (2.0–4.5 px) and alpha (0.4–0.9) scale with shield density | ✅ Done |
| ~~SL-22~~ | ~~Activating shield lines are drawn dimmer (40% alpha) and thinner (50% width) than active lines~~ | ~~Done~~ |
| SL-22a | Activating shield lines are drawn dimmer (60% alpha) and thinner (70% width) than active lines | ✅ Done |
| ~~SL-23~~ | ~~Shield lines are visible when at least one endpoint is visible to the current player~~ | ~~Done~~ |
| SL-23a | Own shield lines are visible when at least one endpoint is visible. Enemy shield lines require both endpoints visible. Remembered (FoW memory) lines show when at least one endpoint is visible | ✅ Done |
...
| SL-42 | Shield line battery support: when a system is attacked, connected shield line neighbors contribute their batteries × `shield_density × SHIELD_BATTERY_SUPPORT_FACTOR` to the defender's pre-combat battery fire. Only neighbors owned by the same player contribute | ✅ Done |
...
| SS-09a | Stations are invisible to enemy players by default. A station becomes visible when: (a) owned by the player, (b) discovered via passive/fleet scan, or (c) garrison weapon signature exceeds enemy scan range. Weapon signature range = `garrison_size × STATION_SIGNATURE_PER_SHIP` pixels. Enemy stars/stations within this range detect the station. Discovery is permanent | ✅ Done |
...
| SS-13a | Stations are destroyed on conquest (not captured). Attacking survivors are also destroyed (no return destination). The combat report shows surviving attackers followed by a loss report stating all survivors were lost with the station | ✅ Done |
...
| SS-38 | When a station is destroyed in combat, the combat report shall include a "SURVIVORS LOST" section after the outcome, showing how many surviving attackers were destroyed with the station (e.g., "5 F / 2 B lost with station") | ✅ Done |
| SS-39 | Fleets arriving at a destroyed station shall generate a loss report for the fleet owner, showing the station name, fleet composition, and stating all ships were lost (target destroyed). Report uses the same combat report UI | ✅ Done |
| SS-40 | Own station info text shall show the current weapon signature range when garrison > 0 (e.g., "Signature: 50px") to inform the player of detection risk | ✅ Done |
| SS-41 | Station placement mode shall show the build signature range (`STATION_BUILD_SIGNATURE`) as expanded enemy scan radii (red overlay includes build signature added to all enemy scan ranges), indicating the combined detection zone for stations under construction | ✅ Done |
...
| SHIELD_BATTERY_SUPPORT_FACTOR | 0.5 (50%) | Neighbor battery contribution factor (× shield density) for shield line battery support |
...
| STATION_SIGNATURE_PER_SHIP | 10 px/ship | Weapon signature range per garrisoned ship (fighter or bomber). Detection range = garrison × this value |
