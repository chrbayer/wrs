...
| SS-40 | Own station info text shall show the current weapon signature range when garrison > 0 (e.g., "Signature: 50px") to inform the player of detection risk | ❌ Not implemented |
| SS-41 | Station placement mode shall show the build signature range (`STATION_BUILD_SIGNATURE`) as a dashed or dimmer circle around the placement cursor, indicating detectability during construction | ❌ Not implemented |
